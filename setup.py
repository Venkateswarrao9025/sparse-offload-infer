...
M9 task 4 (CI): on a machine with no CUDA toolkit (e.g. GitHub Actions'
standard runners), skip building the extension entirely instead of failing
-- `soinfer/__init__.py` already handles `soinfer.ops is None` gracefully
(the quant/offload pure-Python modules work fine without it), so a
pure-Python install lets the CPU-only test files
(test_quant_roundtrip.py, test_hot_cache.py's policy-simulation half)
run in CI. Real Colab/Kaggle T4 builds are unaffected -- CUDA_HOME is set
there, so this still compiles every kernel as before.
"""
import os
...
if HAS_CUDA:
    ...
    print("setup.py: no CUDA toolkit found (CUDA_HOME unset) -- installing soinfer as "
          "pure Python, skipping the _C extension. soinfer.ops will be None; only the "
          "CPU-only test files (test_quant_roundtrip.py, test_hot_cache.py) will work.")
    ext_modules = []
    cmdclass = {}
